...
> First build `data/link-inventory-<client>.csv` from example-dentist.com's sitemap
> (`/sitemap_index.xml`), marking service pages as priority 1. Then follow WORKFLOW.md
> for the blogs below.

(If you already have the client's URL list/crawl, paste or point to it — what you feed is the
source of truth, and Claude will use it instead of crawling.)

Claude will read each live blog, pick verbatim anchors, prioritize service pages, and write
the results to **`output/<client>-recommendations.md`** — in the exact format defined in
`OUTPUT-FORMAT.md` (same shape for every client). Nothing is tied to one website; every file is
per-client.
...
Open `output/<client>-recommendations.md`. For each blog block, copy the `Anchor / Target` lines into
...
- **Few natural anchors on a thin post?** Claude may add a **suggested anchor** (`SUGGEST:` block):
  ...
  **add** to the blog — relevant and natural — so it's listed separately (in `<client>-recommendations.md`
...
- **Every file is per-client.** `data/link-inventory-<client>.csv` holds one client's pages, and
  outputs are `output/<client>-recommendations.md` / `-internal-links.xlsx`. Nothing sits at a
  generic default path, so one client's data can never be reused for another by accident. Start a
  new client from `data/link-inventory.TEMPLATE.csv`.
